...
def read_config()
  File.open("#{WORKINGDIR}/config.txt", "r") do |file|
    config = file.read
    panel_info = []
...
def create_index()
  File.open("#{WORKINGDIR}/index.html", "w+") do | file |
    ...
    sidebar = make_sidebar(all_panels)
    main_area = "<div id=\"main_area\">#{parse_images(WORKINGDIR, all_panels)}</div>"
...
if ARGV[0] == "--build"
  read_config()
  create_index()
  puts "index.html created with assets in current folder"
else
