...
File.open("./test_dist/config.txt", "r") do |file|
  config = file.read
  panel_info = []
  ...
  config.split("\n\n").each do |panel|
    ...
    all_panels << create_panel_hash(panel_info, panel_extras)
  end
end

# File.open("./test_dist/index.html", "w+") do | file |
...
#   dropdowns = make_dropdowns(get_selectors(all_panels))
#   sidebar = "<div id=\"sidebar\"><div id=\"sidebar_footnote\"><span>Everyone ❤️ panels</span></div></div>"
#   main_area = "<div id=\"main_area\">#{parse_images("./test_dist/", all_panels)}</div>"
